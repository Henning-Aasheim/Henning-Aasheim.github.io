...
export default function Home({ params }: { params: Promise<{ locale: string, date: string }> }) {
  ...
  return (
    <div className="text-center pt-12 mb-10">
      ...
      <h1 className="text-2xl sm:text-4xl mb-6 font-default font-bold">{t('header')}</h1>
      ...
      <h1 className="text-2xl sm:text-4xl mb-6 font-default font-bold">{t('blog')}</h1>
      <div className="grid gap-8">
        {allArticlesData.map((article) => (
          <article key={article.id} className="group">
              <Link href={`/blog/${article.id}`}>
              <h2 className="text-lg sm:text-2xl mx-2 sm:mx-0 font-default font-semibold">{article.title}</h2>
